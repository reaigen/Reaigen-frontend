...
function parsePointCloud(buffer: ArrayBuffer): ParsedPointCloud {
  ...
  const sh = [byName.get("f_dc_0"), byName.get("f_dc_1"), byName.get("f_dc_2")];
  const hasRgb = rgb.every(Boolean);
  const hasSh = sh.every(Boolean);
  ...
  for (let index = 0; index < vertexCount; index += 1) {
    ...
    for (let channel = 0; channel < 3; channel += 1) {
      let value = 0.78;
      if (hasRgb) {
        value = scalar(view, base + rgb[channel]!.offset, rgb[channel]!.type) / 255;
      } else if (hasSh) {
        value = 0.5 + SH_C0 * scalar(view, base + sh[channel]!.offset, sh[channel]!.type);
      }
      colors[target + channel] = THREE.MathUtils.clamp(value, 0, 1);
    }
    valid += 1;
  }
