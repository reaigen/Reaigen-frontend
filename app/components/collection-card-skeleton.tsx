...
export function CollectionCardSkeleton({ className }: { className?: string }) {
  return (
    ...
    >
      ...
      <div className="draft-skeleton-shape absolute left-4 top-4 h-7 w-24 rounded-lg sm:left-5 sm:top-5" />
      <div className="absolute inset-x-4 bottom-4 flex items-end justify-between gap-4 sm:inset-x-5 sm:bottom-5">
        <div className="min-w-0 flex-1 space-y-2.5">
          <div className="draft-skeleton-shape h-4 w-[58%] rounded-md" />
          <div className="draft-skeleton-shape h-3 w-[38%] rounded-md opacity-80" />
        </div>
        <div className="draft-skeleton-shape h-9 w-24 rounded-lg" />
      </div>
    </article>
  );
}
