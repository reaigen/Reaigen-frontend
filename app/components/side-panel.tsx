  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        >
          <header className="flex min-h-16 shrink-0 items-start justify-between gap-4 border-b border-border/40 px-5 py-4 sm:px-6">
            ...
            <Dialog.Close asChild>
              <button type="button" aria-label={t("common.close", lang)} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-foreground/40 transition-colors hover:bg-foreground/[0.055] hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
                <CloseIcon size={17} />
              </button>
            </Dialog.Close>
          </header>
          <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5 scrollbar-thin sm:px-6">{children}</div>
          {footer ? <footer className="shrink-0 border-t border-border/40 bg-background/95 px-5 py-3 backdrop-blur-xl sm:px-6">{footer}</footer> : null}
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
  );
